...
- **Shipped** — one trigger + anchored panel surface, unconditional at every breakpoint, on all Standard Page entries (Work and Thoughts both — broader than originally scoped to Thoughts only). Floating trigger + anchored panel shipped for phone/tablet 2026-08-25 after several rounds of real-device fixes; an always-visible Desktop sidebar rail shipped alongside it 2026-08-23 was removed 2026-09-18 (no reserved gutter in the content column, overlapped the entry's own `<h1>`/body copy at Desktop widths) — the trigger + panel pattern now covers Desktop too, unchanged from how it already behaved below 1440px. Panel positioning decoupled from the trigger 2026-09-19: it now anchors a fixed offset below the header/tab-bar (`top: var(--toc-panel-top-offset)`) instead of `bottom`-anchoring to the trigger's own resting position, and targets a real ~75–80% of viewport height via `clamp()` — the prior trigger-anchored formula had a hard ceiling around 45–48% of viewport height no percentage could exceed. Scrollspy tracks scroll position directly (not `IntersectionObserver`) against each heading's own `scroll-margin-top`. Full anatomy, tokens, and states: md/COMPONENTS.md "## 14. Standard Page Template".
...
**2026-09-18**
- **Desktop ToC rail removed; unified onto the trigger + anchored panel pattern at every breakpoint.** Investigated the real current implementation first rather than assuming selector names from a referenced spec doc (`claude/2026-09-18-toc-desktop-collapse-spec.md`, which does not exist anywhere in this repo — same pattern as the two other missing planning docs already flagged 2026-08-23 — likely drafted elsewhere and never committed here). Confirmed via direct read the always-visible Desktop rail (`.toc-rail`, `initTocRail()`/`updateRailTop()`/`updateRailLeft()` in script.js, "STANDARD PAGE — TABLE OF CONTENTS RAIL" section of style.css) was gated at `min-width: 1440px`, not `1024px` — this document's own §6/§8 already had the correct 1440px figure, but the task's own framing guessed 1024px, underscoring why this was investigated rather than assumed. The rail had no reserved gutter in the content column and overlapped the entry's own `<h1>`/body copy at Desktop widths (confirmed the real defect, not just the reported symptom). Removed entirely — markup-building code, both position-tracking functions and their resize/font-load listeners, and all rail-specific CSS (the `.toc-rail`/`.toc-rail-list` rules and the `@media (min-width: 1440px)` block) — rather than fixed in place, per the decision to unify on the existing trigger + anchored panel pattern already shipped for tablet/mobile. That pattern's own CSS (`.toc-panel`, `.toc-panel-scrim`, the trigger's `.action-rail-group.toc-trigger-group` display override) was un-scoped from its former `max-width: 1439px` media query to apply unconditionally at every breakpoint, Desktop included — the narrower `max-width: 767px` tier nested inside it (phone-specific bottom/max-height anchoring) was left untouched. `.toc-rail-link`/`.toc-rail-item--sub`/`.toc-rail-link--active` — shared row classes the panel already used verbatim (Rule 3a) — were kept as-is rather than renamed, since they were never rail-specific markup. Scrollspy (`updateActiveState()`) and heading-ID generation (`getTocHeadings()`) were not touched; only `updateActiveState()`'s display-target loop changed, from `[railLinks, panelLinks].forEach` to `panelLinks.forEach`, since `railLinks` no longer exists. No hardcoded hex values or inline styles introduced (Rule 6) — confirmed via direct grep, not assumed. COMPONENTS.md's "Table of Contents — Desktop Rail" subsection removed outright; "Table of Contents — Mobile/Tablet Trigger & Panel" renamed to "Table of Contents — Trigger & Panel" with its breakpoint-specific wording corrected to reflect unconditional application, plus a dated removal note. This document's own §6 (Standard Page Template status) and §8 (Floating Table of Contents) updated to match. Verified via Playwright/Chrome MCP against `work/this-website.html` served over a local `http.server` (per md's own local-server note — `file://` doesn't work, script.js fetches nav/footer partials): confirmed `document.querySelector('.toc-rail')` returns `null` at 1600px (no rail element exists in the DOM at all, not just CSS-hidden), the trigger renders and opens the same anchored panel at 1600px/834px/390px with no visual differences between the three beyond the pattern's own existing narrower-viewport label collapse, and the `<h1>`/body copy render with no overlap at any width (direct `getBoundingClientRect()` read at 1600px, plus visual confirmation in all three screenshots).

**2026-09-19**
- **ToC panel `max-height` converted to a viewport-relative clamp.** Investigated the real current rule first (per the task's own Step 0) rather than assuming from the referenced spec doc (`claude/2026-09-19-toc-panel-viewport-height-spec.md`, which — same as the two spec docs already flagged 2026-08-23 and 2026-09-18 — does not exist anywhere in this repo). Two things in that doc's own framing didn't match the real code, confirmed via direct read of style.css before changing anything: (1) it describes "the shared Desktop/Tablet `.toc-panel`" and "the separate Mobile bottom-sheet variant" as two distinct components, but there is only one `.toc-panel` selector — the `@media (max-width: 767px)` block is a narrower override tier of the same small anchored popover (its `bottom`/`max-height` retuned to track the trigger's own 70%-from-top anchor at that width, not a different full-width bottom-sheet component); (2) it describes the current value as "a fixed pixel max-height/height," but both tiers already used `vh`-relative `calc()` formulas (`calc(50vh - 64px - --space-6 + --touch-target-minimum/2)` wide tier, `70vh` in place of `50vh` at ≤767px) — not fixed px at all. Treated both tiers (one selector, two media-query-scoped values) as the two "variants" the task meant. Replaced each `calc()` with `min(clamp(--toc-panel-height-min, Nvh, --toc-panel-height-max), <the same original calc()>)` rather than replacing the `calc()` outright: a bare `clamp()` can't safely stand alone here, since its own floor could force the panel taller than the original formula's real header-safety headroom on a short or resized window and overlap the fixed header — confirmed by direct arithmetic (e.g. a naive 320px floor would have overlapped the header below ~644px viewport height at the wide tier). `min()` keeps the original formula as an unconditional safety ceiling while letting the new clamp's own ceiling (`--toc-panel-height-max`, `768px`) additionally cap growth on large/4K-class viewports, where the original formula's unbounded `vh` term would otherwise keep climbing past ~1000px. `--toc-panel-height-min`/`-max` (`320px`/`768px`, `:root`) are `calc(--space-32 * 2.5)` / `calc(--space-32 * 6)` — derived from the largest spacing token per Rule 3a, not arbitrary numbers. Mid-point percentage differs per tier (`46vh` wide, `60vh` at ≤767px) rather than sharing one value — chosen so the clamp stays below the original formula (i.e. non-binding, no regression) at each tier's own realistic viewport heights, confirmed via direct arithmetic and Playwright measurement at four viewports: 384px at 1600×900 and 484px at 834×1100 (wide tier, both identical to the pre-change formula's own output — no regression at ordinary sizes), ~506px at 390×844 (≤767px tier, ~18px under the pre-change formula's ~525px — a negligible reduction, still well above the clamp's 320px floor), and 768px at 3840×2160 (correctly capped, vs. the pre-change formula's own ~1014px at that height) — no overflow or header overlap at any of the four. Confirmed no hardcoded hex values or inline styles introduced (Rule 6) — only token/`calc()`/`clamp()`/`min()` references. Trigger position, open/close transitions, dismiss paths (Escape/scrim/row-click/external-scroll), and scrollspy (`updateActiveState()`) were not touched — verified via Playwright that Escape still closes the panel and returns focus to the trigger, and that scrolling to a new section still updates the active row and the trigger's "X/Y" badge. Applied to both tiers per Rule 5a (not just the wide/Desktop case the task's own context called out) — the ≤767px tier's own mid-point and comment make the parallel, tier-specific reasoning explicit rather than silently reusing the wide tier's numbers. COMPONENTS.md's "Panel positioning"/"Height-cap, scroll, overflow" text updated to describe the new clamp formula and cite the measured values above.
- **ToC panel position decoupled from the trigger; height reworked to a real ~75–80% target.** Supersedes the same-day entry directly above — that fix verified correctly in Playwright but didn't work on the real device it was meant to fix, because 3840×2160 tests a 4K monitor's raw physical resolution, not the ~1260–1300px logical CSS height a real 150%-scaled 4K display actually hands the browser; separately, the panel's `bottom`-anchor to the trigger's own resting position put a hard ceiling around 45–48% of viewport height on any `vh` percentage, since there was never more room than the gap between the header and wherever the trigger sat. Step 0 confirmed (before changing anything): still one `.toc-panel` selector with a `@media (max-width: 767px)` override tier, not two components; the trigger's own `.action-rail-group` sits at `top: 50%` (wide) / `70%` (≤767px), left untouched; `header` (sticky, tablet/desktop) and `.tab-bar` (fixed, mobile) both render at exactly `64px` — one constant covers every tier. Replaced the trigger-relative `bottom`/`max-height` pair with `top: var(--toc-panel-top-offset)` (`calc(64px + --space-6)`, a fixed length, not `vh`-based) plus `height: min(clamp(--toc-panel-height-min, 78vh, --toc-panel-height-max), calc(100vh - --toc-panel-top-offset - --space-6))` — `height`, not `max-height`, since the spec wants a real presence-sized target, not a shrink-to-content ceiling (a short heading list now shows empty space below its last row, an intentional trade). `--toc-panel-height-min`/`-max` raised from the prior session's `320px`/`768px` to `320px`/`1024px` (`calc(--space-32 * 2.5)` / `calc(--space-32 * 8)`, Rule 3a) — the ceiling's role shifted from "keep the panel modest on huge displays" to "bound an otherwise-unbounded 78vh on a hypothetically very tall window," so it needed to sit well above the new 78%-of-ordinary-heights target rather than below it. `transform-origin`/`translateY` flipped from `bottom right`/positive offset to `top right`/negative, matching the new top-anchored open direction. The Mobile `<=767px` "Open Item" the task flagged: investigated and confirmed no override is needed — removed the tier's `.toc-panel` block entirely, since the header/tab-bar height it would anchor to is identical at every breakpoint; one unconditional rule now covers all three. Verified via Playwright at the four spec-mandated sizes, all logical CSS viewport heights rather than physical monitor resolutions: 2560×1300 (the 150%-scaled-4K stand-in), 1920×1000, 834×1100, 390×844 — every one landed at exactly 78.0% of its own viewport height, confirmed via `getBoundingClientRect()`, with zero header/tab-bar overlap and zero viewport overflow at any of them; also confirmed the panel's vertical span no longer intersects the trigger's own position at 2560×1300 (trigger at y≈628–672, panel spanning y≈88–1102). Escape-close-with-focus-return and scrollspy (badge + active row, scrolled to a new heading) reconfirmed unchanged. Confirmed no hardcoded hex values or inline styles introduced (Rule 6) — only token/`calc()`/`clamp()`/`min()` references. COMPONENTS.md's "Panel positioning" and "`max-height` clamp" text (both from the entry directly above) rewritten to describe the new anchor/height design, including a new subsection recording the Mobile tier finding.
- **Follow-up same day: `.toc-panel`'s `height` reverted to `max-height`.** The entry directly above shipped `height` (a fixed target) rather than `max-height` (a ceiling) for the decoupled-position panel, so a page with a short heading list rendered visible empty space below its last row — the box was forced to `~78vh` regardless of how much content it actually held. Swapped `height` back to `max-height` with the exact same clamp/`calc()` value otherwise unchanged (`min(clamp(--toc-panel-height-min, 78vh, --toc-panel-height-max), calc(100vh - --toc-panel-top-offset - --space-6))`) — nothing else from the decoupled-position change (the `top` anchor, the min/max token values, `transform-origin`/`translateY`) was touched. Verified via Playwright: `thoughts/physical-and-digital-media.html` (5 headings, an actual short list — the task's own suggestion of `this-website.html` as a short-list example was checked first and found to be wrong, confirmed via direct heading count: it has 21 headings, not few) now shrink-wraps to 289px (28.9% of a 1920×1000 viewport) with zero trailing gap below the last row (the only space below content is the panel's own `--space-3` bottom padding, not a bug); `work/star-engine.html` (19 headings, the long list named in the task) still caps at exactly 780px (78.0% of the same 1920×1000 viewport) with `scrollHeight` (956px) exceeding `clientHeight`, confirming internal scroll is still engaged, unchanged from the decoupled-position verification. COMPONENTS.md's `height` clamp paragraph corrected to `max-height` and rewritten to describe both the brief `height` regression and the fix.

**2026-09-20**
- **Thoughts entry "Read Me" renamed to "Welcome: My AI Use and Ethics" (slug `read-me` → `welcome`), with a 301 from the old live URL.** The site has been live at crzdevz.com (Netlify) since 9/8, so the old URL is redirected rather than dropped. `git mv entries/thoughts/read-me.html entries/thoughts/welcome.html` (shows as a rename, history preserved, no copy left behind). New title updated byte-identically in the entry's `<title>` (`Welcome: My AI Use and Ethics — Thoughts — Christopher Klein`), breadcrumb last segment and `<h1>`; in Home's hand-written Thoughts card (`href`s, `<h3>`, CTA `aria-label`, block-link `aria-label` now `Read Welcome: My AI Use and Ethics`); and in `data/archive-entries.json` (`id` → `welcome`, `title`, `url`). New `_redirects` at the repo root (no `netlify.toml` or `_redirects` existed before; there is no build step and root-relative asset paths confirm the repo root is the publish directory) with two non-forced 301s: `/entries/thoughts/read-me.html` → `/entries/thoughts/welcome.html` and `/entries/thoughts/read-me` → `/entries/thoughts/welcome`. Both forms are needed — read-only HEAD checks against the live site before the change showed `/entries/thoughts/read-me.html` and `/entries/thoughts/read-me` both return 200 with no redirect between them. Non-forced is deliberate: Netlify won't apply a plain rule over an existing file, and the `git mv` removes the old one. No redirect added for the earlier slugs (`code-and-conduct`, `thrilling-beginnings`) — `code-and-conduct.html` already 404s live, per decision. The manifest `blockLinkLabel` override existed only to avoid "Read Read Me"; removed for this entry only (no other entry set it — confirmed via repo-wide grep), so the default `{Read|View} {title}` convention now applies. The field is still supported in `buildCard()` (script.js) and is now an **optional** manifest field with no current users; the comment above it was reworded to a generic example (comment only, no code change). Deliberately untouched, per scope: body copy, `excerpt`, meta description, `image`/`imageAlt`, tags, the Updated date. The entry's image folder (`assets/images/entries/thoughts/thrilling-beginnings/`) still carries a legacy slug — it never embedded `read-me`, Thoughts cards render no images, and it was left as-is per decision. Flagged, not fixed: the §4 file tree above still lists `work/`, `thoughts/` and `templates/` at the repo root; the real paths have been `entries/work/`, `entries/thoughts/` and `entries/templates/` since the 9/5 move. NEW-ENTRY-PROCESS.md's "Renaming a slug" gained a sentence on live-site 301s; COMPONENTS.md and this document's live-entry path lists updated. Redirects cannot be exercised on a local static server — verify on the Netlify dev preview after pushing (old URL `/entries/thoughts/read-me.html`, expect 301 to `/entries/thoughts/welcome.html`).
